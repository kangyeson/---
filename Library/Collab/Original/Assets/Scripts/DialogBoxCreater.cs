...
public class DialogBoxCreater : MonoBehaviour
{
    ...
    private void Create()
    {
        GameObject dialogbox = Instantiate(DialogPrefab, new Vector3(543,280, 0), Quaternion.identity);
        dialogbox.name = "DialogBox";
        GameObject maincomponents = GameObject.Find("Canvas/Panel/MainComponents");
        dialogbox.transform.SetParent(maincomponents.transform);

        //대사 부여
        DialogueTrigger dialogObject = new DialogueTrigger();
        var info = dialogObject.info;
        info.names.Add("유저");
        info.names.Add("영재");
        info.names.Add("유저");
        info.names.Add("영재");
